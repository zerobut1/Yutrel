...
namespace Yutrel
{
    Ref<CameraController> CameraController::Create(float aspectRatio, glm::vec3 positon, float yaw, float pitch)
    {
        LOG_INFO("Create a CameraController");
        return CreateRef<CameraController>(aspectRatio, positon, yaw, pitch);
    }
          ...
          m_pitch(pitch)
    {
        ...
        // 滚轮缩放视角和右键按住拖动视角
        // auto window = *Application::GetInstance().GetWorld().GetResource<Ref<Window>>();
    ...
    // {
    //     auto input     = *Application::GetInstance().GetWorld().GetResource<Ref<Input>>();
    ...
    void CameraController::UpdateCameraVectors()
    {
        ...
        m_camera->setDirection(m_camera_direction);
        m_camera->setUp(m_camera_up);
        m_camera->SetRight(m_camera_right);
    }
} // namespace Yutrel