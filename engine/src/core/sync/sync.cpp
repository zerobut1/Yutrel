...
namespace Yutrel
{
    ...
    constexpr uint64_t warn_threshold = static_cast<GLuint64>(1e10); // 10 seconds

    Sync::Sync(GLuint id) : id(id)
    {
        sync = glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0);
        YUTREL_ASSERT(sync != nullptr, "Unable to create a fence sync object");
    }
    ...
    void Sync::ClientWaitSync(GLuint64 timeout)
    {
        ...
        while (status == GL_TIMEOUT_EXPIRED)
        {
            ...
            {
                LOG_WARN("Sync object {0} has been hanging for over 10 secs on the client!", id);
                warned = true;
            }
        }
        ...
        {
            LOG_ERROR("An error occurred while waiting on sync object {0}", id);
        }
    }
    ...
    void Sync::ServerWaitSync()
    {
        ...
        while (!Signaled())
        {
            ...
            if (!warned && wait_time > warn_threshold)
            {
                LOG_WARN("Sync object {0} has been hanging for over 10 secs on the server!", id);
                warned = true;
            }
        }
    }

    int64_t Sync::GetServerTimeout()
    {
